...
if __name__ == '__main__':
    start = time.time()
    df1 = pd.read_csv('2013to2015Data_merged.csv', encoding="latin-1")
    df2 = pd.read_csv('2016to2019Data_merged.csv', encoding="latin-1")
    df1 = df1.append(df2)
    ...
    top_3_list = []
    print("Imports done in {}".format((time.time()-start)/60))
    ...
    for day in day_list:
        for season in season_list:
            for team in team_list:
                ...
                if all(elem in list(day_df['slug']) for elem in team_top_3_list):
                    ...
                    if len(C_df_sorted) == 0:
                        ...
                        top_C = list(C_df_sorted.head(1)['slug'])[0]
                        day_df = day_df[day_df['player'] != top_G]
                        day_df = day_df[day_df['player'] != top_C]
                        day_df_sorted = day_df.sort_values(['game_score'], ascending=False)
                        other_top = list(day_df_sorted.head(1)['slug'])[0]
                        team_top_3_list = []
                        team_top_3_list.append(top_G)
                        ...
                        count += 1
    print("Imports done in {}".format((time.time()-start)/60))
    game_df.to_csv('GameClassifier_new1.csv', header=True)
