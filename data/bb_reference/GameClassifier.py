...
if __name__ == '__main__':
    df1 = pd.read_csv('2016to2019DataMAC.csv',encoding = 'latin-1')
    ...
    for team in team_list:
        for season in season_list:
            ...
            team_df_grouped = team_df.groupby(['slug']).sum()
        ...
        for season in season_list:
            for team in team_list:
                ...
                if all(elem in list(day_df['slug']) for elem in team_top_3_list):
                    ...
                    if len(list(day_df_sorted.head(3)['slug'])) == 3:
                        team_top_3_list = list(day_df_sorted.head(3)['slug'])
                        team_top_3_list.append(team)
                        team_top_3_list.append(day)
                        team_top_3_list.append(list(day_df.location)[0])
                        game_df.loc[count] = team_top_3_list
                        count += 1
    game_df.to_csv('games_classified.csv')
